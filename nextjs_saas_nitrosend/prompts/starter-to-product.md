...
1. Starter audit
   ...
   - Also inspect `app/auth/sync/route.ts`, `app/api/webhooks/stripe/route.ts`, `lib/nitrosend-config.ts`, and `lib/nitrosend-email.ts` to see the Nitrosend lifecycle email integration wired to sign-in and the Stripe webhook.
...
4. Phased implementation
   ...
   - Nitrosend lifecycle email is already wired in through `lib/nitrosend-email.ts`, sending from the hosted sender Stripe Projects provisioned (`NITROSEND_API_KEY`, `NITROSEND_FROM_ADDRESS`). Treat this as a real product surface, not scaffolding:
     - A first sign-in sends `welcome` from `app/auth/sync/route.ts`. Both the Stripe webhook (`app/api/webhooks/stripe/route.ts`) and the success page (`app/success/page.tsx`) send `subscription_active` through `sendWelcomeEmailForSubscriptionOnce`, which claims an atomic guard in the database (`welcome_email_sent_at` on `subscriptions`) so exactly one email goes out per subscription even if both paths fire at once. The webhook also sends `trial_ending` and `payment_failed`.
     - Rewrite the subjects and bodies in `buildLifecycleEmailContent` to match the product name, voice, and offer instead of the generic starter copy.
     - Consider other lifecycle moments the product needs: receipts, onboarding sequences, plan changes and renewal notices, account notifications. Add a `LifecycleEmailKind` and call `sendLifecycleEmail` with a stable idempotency key.
     - Keep sends best-effort so an email failure never blocks auth, checkout or the webhook.
     - For campaigns, flows, segments and audience management, connect the Nitrosend MCP server (`claude mcp add --transport http nitrosend "$NITROSEND_MCP_URL"`) and let it author them; do not build email marketing into the app.
     - The home page shows a Nitrosend Email status card (`app/page.tsx`) that flips to setup instructions when `NITROSEND_API_KEY` is unset. Remove it from the product experience along with the other starter status panels.

5. Verification
   - Verify `/api/health` still reflects the app correctly, including `nitrosendEmailConfigured`.
   - Verify the main user flow still works for the integrations that are present.
   - If Nitrosend email copy changed, verify a first sign-in sends the welcome email, a completed test subscription still sends the subscription-active email once, and the webhook still returns 200.
