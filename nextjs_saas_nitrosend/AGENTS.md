...
- Preserve the Nitrosend email integration in `lib/nitrosend-email.ts`: the welcome email on first sign-in (`app/auth/sync/route.ts`), the subscription-active email sent from both the Stripe webhook and the success page through `sendWelcomeEmailForSubscriptionOnce` (deduped by an atomic database guard), and the trial-ending and payment-failed emails from the webhook. Every send carries an idempotency key and is best-effort so it never blocks auth, checkout or the webhook. Customize the copy in `buildLifecycleEmailContent` rather than removing the wiring.
- For campaigns, flows, segments and audience work, use the Nitrosend MCP server (`NITROSEND_MCP_URL`, guide at https://nitrosend.com/agents/provision.md) instead of hand-coding email marketing into the app.
...
- If Nitrosend email copy changed, verify a first sign-in sends the welcome email, a completed test subscription sends the subscription-active email once, the webhook still returns 200, and `/api/health` still reports `nitrosendEmailConfigured`.
